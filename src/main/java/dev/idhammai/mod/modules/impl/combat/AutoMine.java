...
for (Direction dir2 : Direction.values()) {
    ...
    BlockPos side = target.offset(dir2);
    ...
    if (canCrystal(side)) {
        ...
        for (Direction dir3 : Direction.values()) {
            ...
            BlockPos side2 = side.offset(dir3);

            if (mc.world.isAir(side2)) {
                PacketMine.INSTANCE.mine(side2);
            }
        }
        return;
    }

    if (!mc.world.isAir(side)) {
        PacketMine.INSTANCE.mine(side);
        return;
    }
}
...
if (!mc.world.isAir(side)) {
    ...
    return down == Blocks.OBSIDIAN || down == Blocks.BEDROCK;
}
}
